...
#add_action('wp','sdco_redirect_work');

function sdco_redirect_work(){

	if (is_post_type_archive('work') && !is_admin()) {
	
		$work = new WP_Query(array('post_type' => 'work','posts_per_page' => 1));
		$post = $work->posts[0];
...
add_action('pre_get_posts','sdco_adjust_query');

function sdco_adjust_query($query)
{
	
	if ($query->is_main_query() && !$query->is_admin && is_post_type_archive('work')) {
